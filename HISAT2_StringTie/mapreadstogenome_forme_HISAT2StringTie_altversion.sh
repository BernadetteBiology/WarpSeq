...
# Pipeline: Trimmomatic > HISAT2 > STRINGTIE (alternative version)
# Requirements (Pre-req programs): Trimmomatic, HISAT2, STRINGTIE (any version), SAMTOOLS, GFFREAD, PARALLEL, DOS2UNIX, Python 2.7
# Requirements (In directory): 	
 ...
 # e. The prepDE.py script from StringTie.
 # f. This program.
 ...
 # Important:
    ...
    # This program allows for the version of StringTie being used to be specified. See "DEFAULT_STRINGTIEV=" to input the path to the version of StringTie you would like to use. Since this
    # typically means an older version of StringTie, Python 2.7 is used in the count matrix generation step. See "generate_counts_table()" to change version of Python.

# --- Default Configurations ---
DEFAULT_TRIMMOMATIC_JAR="./trimmomatic-0.36.jar"
...
DEFAULT_STRINGTIEV="$HOME/path/to/stringtie-v.v.v/stringtie" # Please change this to reflect the path of the specific version of StringTie you would like to use.
...
usage() {
    ...
    echo "  -s, --stringtie-v PATH    Path to StringTie program (default: $DEFAULT_STRINGTIEV)"
...
run_hisat2() {
    ...
    parallel -j "$PARALLEL_JOBS" hisat2_process :::: "$TEMP_DIR/basenamereads.txt"
...
run_stringtie_merge() {
    ...
    mkdir -p "${OUTPUT_DIR}/stringtie"
    cd "${OUTPUT_DIR}/stringtie" || exit 1

    # Generate a list of GTF files
    ls *.gtf > gtf_names.txt

    if $USE_ANNOTATION; then
        echo "Merging with annotation: ${GENOME_DIR}/annotatedgenome_gffread.gff3"
        "$STRINGTIE" --merge \
            -G "${GENOME_DIR}/annotatedgenome_gffread.gff3" \
            -o merged_transcripts.gff \
            gtf_names.txt
    else
        echo "Merging without annotation"
        "$STRINGTIE" --merge \
            -o merged_transcripts.gff \
            gtf_names.txt
    fi
...
# Generate counts table using prepDE.py
generate_counts_table() {
    echo "Generating counts matrix using prepDE.py..."
    if [[ ! -f "prepDE.py" ]]; then
        echo "Error: prepDE.py not found in the current directory." >&2
        exit 1
    fi
